			...
			for (int z = 0; z < 16; z++)
			{
				...
				for (int y = bottomHeight + 1; y > bottomHeight && y < maxHeight && y < 255; y++)
				{
					...
					if (thisblock != (decimal) Material.Air && blockabove == (decimal) Material.Air)
					{
						...
						if (chunk.GetBlock(x, y - 1, z) != (decimal) Material.Air) chunk.SetBlock(x, y - 1, z, (byte) Material.Dirt);
						if (chunk.GetBlock(x, y - 2, z) != (decimal) Material.Air) chunk.SetBlock(x, y - 2, z, (byte) Material.Dirt);
					}
				}
