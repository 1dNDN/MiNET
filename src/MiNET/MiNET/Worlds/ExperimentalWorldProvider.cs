	...
	internal class ExperimentalWorldProvider : IWorldProvider
	{
		...
		private void PopulateChunk(ChunkColumn chunk)
		{
			...
			for (int z = 0; z < 16; z++)
			{
				...
				if (stoneHeight < stoneMinHeight) stoneHeight = (int) Math.Floor(stoneMinHeight);
				...
				for (int y = 0; y < 256; y++)
				{
					...
					if (y <= stoneHeight)
					{
						...
						//Diamond ore
						if (GetRandomNumber(0, 2500) < 5) chunk.SetBlock(x, y, z, 56);

						//Coal Ore
						if (GetRandomNumber(0, 1500) < 50) chunk.SetBlock(x, y, z, 16);

						//Iron Ore
						if (GetRandomNumber(0, 2500) < 30) chunk.SetBlock(x, y, z, 15);

						//Gold Ore
						if (GetRandomNumber(0, 2500) < 20) chunk.SetBlock(x, y, z, 14);
					}
					...
					if (y < waterLevel) //FlowingWater :)
					{
						...
						if (y < waterLevel - 3) chunk.SetBlock(x, y + 1, z, 8); //FlowingWater
					}
			...
			int z)
		{
			...
			for (int i = 0; i <= treeheight; i++) chunk.SetBlock(x, treebase + i, z, 17);
		}
